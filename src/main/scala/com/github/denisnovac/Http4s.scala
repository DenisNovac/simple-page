...
object Http4s:
  def of[F[_]: Async](tapirRoutes: List[ServerEndpoint[Any, F]]): Resource[F, org.http4s.server.Server] =
    ...
    val routes = Http4sServerInterpreter[F](options).toRoutes(tapirRoutes)
